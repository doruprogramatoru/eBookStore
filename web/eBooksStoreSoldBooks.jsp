<%-- 
    Document   : eBooksStoreSoldBooks.jsp
    Author     : doru
--%>
            ...
            <c:when test="${validUser == true}">  
                ...
                <form action="${pageContext.request.contextPath}/eBooksStoreSoldBooksServlet" method="POST">
                    ...
                    <sql:query dataSource="${snapshot}" var="result">
                        SELECT SOLD_EBOOKS.USER_ID, SOLD_EBOOKS.USER_NAME, SOLD_EBOOKS.ISBN,  SOLD_EBOOKS.TITLE, SOLD_EBOOKS.CATEGORY, SOLD_EBOOKS.NAME, SOLD_EBOOKS.ACQUIRED_DATE, SOLD_EBOOKS.ACQUIRED_QUANTITY, SOLD_EBOOKS.BOUGHT_DATE, SOLD_EBOOKS.BOUGHT_QUANTITY FROM SOLD_EBOOKS, EBOOKS, USERS WHERE SOLD_EBOOKS.USER_ID = USERS.ID AND SOLD_EBOOKS.ISBN = EBOOKS.ISBN ORDER BY USER_ID ASC
                    </sql:query>
                    <table border="1" width="100%">
                        <tr>
                            ...
                            <%-- <td width="24%" class="thc">BOUGHT_QUANTITY</td> --%>
                    </table>    
                    <table border="1" width="100%">    
                        </tr>
                        <c:forEach var="row" varStatus="loop" items="${result.rows}">
                            <tr>
                                ...
                                <%-- <td width="24%" class="tdc"><c:out value="${row.bought_quantity}"/></td> --%>
                            </tr>
                        </c:forEach>
                    </table>
                </form>
